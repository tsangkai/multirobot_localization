...
def simulation():
	...
	for i in range(num_of_trial):
		...
		sigma_tr_arr += math.sqrt(total_sigma.trace()[0,0]/float(N)) / float(num_of_trial)
		...
		for j in range(N):
			...
			total_sigma = robots[j].getSigma()
			tr_error += total_sigma[2*j:2*j+2,2*j:2*j+2].trace()[0,0]
	...
	# file = open('topology_result/gs_sci_output.txt', 'w')
	# for t in range(total_T):
